...
namespace cypos
{
    public partial class CurPad : UserControl
    {
        public CurPad()
        {
            InitializeComponent();
        ...
        public void Start_Events()
        {
            ...
            this.btnDecimal.Click += new System.EventHandler(this.btnDecimalClicks);
        ...
        void btnDecimalClicks(object sender, EventArgs e)
        {
            Button button = sender as Button;

            string buttonText = button.Text;
            if (!KeyPadReturnValue.Contains("."))
                KeyPadReturnValue = KeyPadReturnValue + buttonText;
            if (!this.Focused)
                this.Focus();
        }
